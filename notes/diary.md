...
在单独引入一个组件时，js会通过 `components.js` 去分包进行加载，但所有组件的样式都是自执行函数，在进行分包分析时就会自动执行，将该组件的样式挂载到`head`中

这就导致了引入一个组件，却挂载了所有组件的样式。

想法： 
  只有在加载组件时才去挂载样式，
 1. `Css In Js`，只为了这一个功能点而去改变整体方案，好像没有必要。 当前的 `Css In Js` 方案没有找到比较适合的。

 2. 通过插件，在组建打包时将自执行函数写入到组件挂载函数中，并标记上载入的`style`标签，以免多次挂载该组件而导致挂载上多个`style`